...
Renderer = function(container, ui) {
  ...
  animate = function() {
    setTimeout(animate.bind(_this), 100);
    _this.render();
    if (_this.stats) {
      return _this.stats.update();
    }
  };
  this.onGcodeLoaded = function(gcode) {
    var gcodeObj;
    _this.gr = new GCodeRenderer;
    _this.gm = ParseGCode(gcode);
    gcodeObj = _this.gr.render(_this.gm);
    window.gr = _this.gr;
    window.r = _this;
    _this.ui.controllers.gcodeIndex.max(_this.gr.viewModels.length - 1);
    ...
    _this.camera.lookAt(_this.gr.center);
    console.log(gr);
    if (_this.object) {
