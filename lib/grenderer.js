...
// Model for a GCode
function GCodeViewModel (code, geometryHandlers, materialHandlers) {
  this.code = code;
  this.vertexIndex = 0;
  this.vertexLength = 0;
  ...
  var geometryHandler = geometryHandlers[cmd];
  var materialHandler = materialHandlers[cmd];
  if (geometryHandler) {
    geometryHandler(this);
  }
  if (materialHandler) {
    materialHandler(this);
  }
}
...
function GCodeRenderer() {
  ...
  this.geometryHandlers = {
    ...
    G1: function(viewModel) {
      ...
      var color1 =  new THREE.Color(feedColors[viewModel.code.index%feedColors.length]);
      var color2 =  feedColors[viewModel.code.index%feedColors.length];
      ...
      if( viewModel.code.index <= self.index ) {
        ...
        self.feedGeo.colors.push(color2);
        self.feedGeo.colors.push(color1);
      } else {
      ...
      self.feedAllGeo.colors.push(color2);
      self.feedAllGeo.colors.push(color1);
...
GCodeRenderer.prototype.render = function(model) {
  ...
  this.model.codes.forEach(function(code) {
    self.viewModels.push(new GCodeViewModel(code, self.geometryHandlers, self.materialHandlers))
  });
  ...
  console.log(scale, 'oi')

  // WTF: bizarre result wo this deffer
  setTimeout(function () {
