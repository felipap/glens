...
function GCodeRenderer() {
  ...
  this.geometryHandlers = {
    ...
    G1: function(viewModel) {
      ...
      // var color1 =  new THREE.Color(GCodeRenderer.feedColors[viewModel.code.index%GCodeRenderer.feedColors.length]);
      // var color2 =  GCodeRenderer.feedColors[viewModel.code.index%GCodeRenderer.feedColors.length];
      ...
      if( viewModel.code.index <= self.index ) {
        ...
        self.feedGeo.colors.push(color); // color2);
        self.feedGeo.colors.push(color); // color1);
      } else {
        self.feedIncGeo.colors.push(color); // color2);
        self.feedIncGeo.colors.push(color); // color1);
      ...
      self.feedAllGeo.colors.push(color); // color2);
      self.feedAllGeo.colors.push(color); // color1);
...
GCodeRenderer.prototype.render = function(model) {
  ...
  // for (var i=0; i<Math.min(10000000, this.model.codes.length); ++i) {
  //   this.renderGCode(this.model.codes[i])
  // }

  this.model.codes.forEach(function(code) {
    self.viewModels.push(self.renderGCode(code))
  });
  ...
  setTimeout(function () {
    this.baseObject.applyMatrix(
      new THREE.Matrix4().makeTranslation(this.center.x, this.center.y, this.center.z) );
  }.bind(this), 1000)
...
GCodeRenderer.prototype.renderGCode = function (code) {
  ...
  var geometryHandler = this.geometryHandlers[cmd];
  var materialHandler = this.materialHandlers[cmd];
  if (geometryHandler) {
